...
class EquiformerV2ForcesTrainer(ForcesTrainer):
    ...
    def load_model(self):
        ...
        fine_tune = self.config["optim"].get("fine_tune", False)

        if fine_tune:
            self.finetune_output_layers()
    ...
    def finetune_output_layers(self):
        """ Fine-tune only the energy and force output layers, freeze all other layers. """
        # Freeze all layers first
        for param in self.model.parameters():
            param.requires_grad = False

        if self.config["optim"]["energy_block"]:
            for param in self.model.energy_block.parameters():
                param.requires_grad = True

        if self.config["optim"]["force_block"]:
            for param in self.model.force_block.parameters():
                param.requires_grad = True

        for block_id in self.config["optim"]["conv_block"]:
            for param in self.model.blocks[block_id].parameters():
                param.requires_grad = True

        logging.info("-----------------------------------------\n"
                     "Fine-tuning mode.\n"
                     "The following layers are trainable: \n"
                     f"conv_block: {self.config['optim']['conv_block']}, \n"
                     f"energy_block: {self.config['optim']['energy_block']}, \n"
                     f"force_block: {self.config['optim']['force_block']}.\n"
                     "-----------------------------------------\n")
