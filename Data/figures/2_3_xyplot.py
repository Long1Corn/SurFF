...
def xy_plot(df, save_pth=None):
    ...
    # text mae
    ax_scatter.text(0.95, 0.05, f"MAE: {mae:.1f} meV/A$^2$", transform=ax_scatter.transAxes, fontsize=18,
                    fontweight='bold',
...
if __name__ == '__main__':
    df = pd.read_csv(
        r"D:\OneDrive\OneDrive - National University of Singapore\Pyprojects\Intermetallic_Crystal_Nanoparticle_Model\results\element_id\wulff_results.csv"
    )
    xy_plot(df, save_pth=
    r"D:\OneDrive\OneDrive - National University of Singapore\Pyprojects\ocp\results\element_id\wulff_results.png"
            )
